...
$id = $_GET['uid'];
 if (!isset($_POST['submit']))  {
    ...
    $status = $_POST['status'];
        ...
        $sql = "UPDATE `careers` SET title = '$title', description = '$description', status='$status' WHERE id = '$id' ";
        if($conn->query($sql) === TRUE){
           echo "<script>alert('Data Updated Successfully');window.location.href='careers.php';</script>";
        } else {
           echo "<script>alert('Data Updation Failed');window.location.href='careers.php';</script>";
        }
    }   
                ...
                <div class="card-content">                                
                    <div class="row">
                        <form class="col s12" method="post" enctype="multipart/form-data">
                            <div class="row">
<?php $getContents = getAllDataWhere('careers', 'id', $_GET['uid']); $getContents1 = $getContents->fetch_assoc(); ?>
